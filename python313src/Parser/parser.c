...
//根据PyTokenizer所获得的token和当前所处的状态/DFA，跳转到下一个状态，并添加到CST中
        ...
        if (s->s_lower <= ilabel && ilabel < s->s_upper) {
            ...
            if (x != -1) {
                if (x & (1<<7)) {
                    /* Push non-terminal */
                    int dfa_id_ = x >> 8;
                    //int nt = (x >> 8) + NT_OFFSET; // nt - node type
                    int nt = dfa_id_ + NT_OFFSET; // nt - node type
                    int arrow = x & ((1<<7)-1);
                    //dfa *d1 = PyGrammar_FindDFA(ps->p_grammar, nt);
                    //dfa *d1 = & ps->p_grammar->g_dfa[nt-NT_OFFSET];
                    dfa *d1 = & ps->p_grammar->g_dfa[dfa_id_];
                    assert(d1->d_type==nt);
